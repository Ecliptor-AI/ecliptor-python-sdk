...
class Ecliptor:
    ...
    def adapt(self, embedding: List[float]) -> Dict[str, Any]:
        """
        ...
        try:
            response = self.session.post(f"{self.base_url}/adapt", json=data)
            response.raise_for_status()
            return response.json()["embedding"] # Just return the list of floats
